...
开始菜单快捷方式也不保存带版本号的 `WindowsApps` 路径：启动目标固定为插件安装目录中的 `scripts\start.ps1`，图标固定为 `assets\Codex.ico`。安装器只在安装时按 Windows 应用主题复制当前 Codex 自带的官方 ICO；此后的 Codex 版本更新不会让快捷方式引用失效。桌面上已有且属于本插件的 `Codex（用量显示）` 快捷方式会同步刷新图标，但安装器不会擅自新增桌面入口。切换 Windows 应用主题后，可重新运行安装器以同步官方主题变体。

## 安装

在 `windows` 目录使用 PowerShell 7 安装：
...
在 `windows` 目录使用 PowerShell 7 验证：
...
确认不再需要该插件后，在 `windows` 目录使用 PowerShell 7 卸载：
